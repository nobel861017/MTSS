...
def gen_mix_list_tr(args, accents, accent_wav_dict, output_filename):
    ...
    for ac in accents:
        ...
        spk_mix_pair_list = [(spk_list[i], spk_list[j], spk_list[k]) for i in range(len(spk_list)) for j in range(i+1, len(spk_list)) for k in range(j+1, len(spk_list))]
        spk_mix_pair_list = random.sample(spk_mix_pair_list, 2222)
        total_tasks += len(spk_mix_pair_list)
        for _, (spk1, spk2, spk3) in enumerate(tqdm(spk_mix_pair_list)):
            for i in range(args.k):
                for j in range(args.k):
                    for k in range(args.k):
                        db = round(random.uniform(0.0, 2.5), 6)
                        string = ' '.join([spkr_utt_dict[spk1][i], str(db), spkr_utt_dict[spk2][j], str(-db), spkr_utt_dict[spk3][k], '0' ])
...
def gen_mix_list(args, accents, accent_wav_dict, output_filename):
    ...
    for ac in accents:
        ...
        spk_mix_pair_list = [(spk_list[i], spk_list[j], spk_list[k]) for i in range(len(spk_list)) for j in range(i+1, len(spk_list)) for k in range(j+1, len(spk_list))]
        total_tasks += len(spk_mix_pair_list)
        for _, (spk1, spk2, spk3) in enumerate(spk_mix_pair_list):
            for i in range(args.k):
                for j in range(args.k):
                    for k in range(args.k):
                        db = round(random.uniform(0.0, 2.5), 6)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--spknum', type=int, default=8, help='maximum number of speakers to use for each task')
    ...
    for ac in accents_all:
        ...
        if len(os.listdir(accent_path)) < 3: continue
        accent.append(ac)
    ...
    if args.do_tr:
        print('gen train mix list')
        gen_mix_list_tr(args, ['english'], accent_wav_dict, 'mix_3_spk_tr.txt')
    if args.do_cv:
        print('gen valid mix list')
        gen_mix_list(args, accent[:int(accent_num*0.5)], accent_wav_dict, 'mix_3_spk_cv.txt')
    if args.do_tt:
        print('gen test mix list')
        gen_mix_list(args, TEST_ACCENTS, accent_wav_dict, 'mix_3_spk_tt.txt')
